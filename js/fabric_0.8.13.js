    ...
    function renderText(color) {
      ...
      for (var i = 0, l = chars.length; i < l; ++i) {
        ...
        if (textDecoration) {
          ...
          // add 2x more thickness ‰ÛÓ closer to SVG rendering
...
(function() {
  ...
  var InteractiveMethods = /** @scope fabric.Canvas.prototype */ {
     ...
     * @type String
     */
    selectionColor:         'rgba(100, 100, 100, 0.3)', // blue
...
(function(global) {
  ...
  fabric.Object = fabric.util.createClass(/** @scope fabric.Object.prototype */ {
    ...
    stateProperties:  ('top left width height scaleX scaleY flipX flipY ' +
                      ...
                      'strokeWidth fillRule borderScaleFactor transformMatrix fillColor letterSpacing radius borderColor isBorder ' +
                      'selectable').split(' '),
    ...
    cornersize:               12,
    padding:                  0,    
    cornerColor:              'rgba(102,102,102,0.5)',
    fill:                     'rgb(0,0,0)',
...
/*		
	rsgBuffer: function(value) {
			var imgObj = document.getElementById('bufImage');
...
(function(global) {
  ...
  fabric.Image = fabric.util.createClass(fabric.Object, /** @scope fabric.Image.prototype */ {
    ...
    initialize: function(element, options) {
	  ...
	  this.initializeColor(options);	  

	  if(options && options.isUploadedImage)
		  this.isUploadedImage = !!options.isUploadedImage;
	...
	initializeColor: function(options) {	
		...
		if(options.originalColors == undefined) {				
			...
			for (var i = 0, len = data.length; i < len; i += 4) {
				if(data[i+3] != 0) {
					var b = true;
					var hex = rgbtohex3(data[i], data[i+1], data[i+2]);
					
					for(var j = 0; j < this.originalColors.length; j++) {		
						if(this.originalColors[j] == hex) b = false;
					}
					if(b) {
						this.originalColors[inx] = hex;
						this.currentColors[inx] = hex;
						inx++;

						if(inx > 8) {
							/*
							this.originalColors = [];
							this.originalColors[0] = '000000';
							this.currentColors = [];
							this.currentColors[0] = '000000';
							*/
							alert("It has too many colors to be converted.");
							return;
						}
					}
				}
			...
			if(this.originalColors.length == 1) {
				this.originalColors[0] = '000000';
				this.currentColors[0] = '000000';
			}
		} else {
			this.originalColors = options.originalColors;
			this.currentColors = options.currentColors;
		}
	},
	...
	setChangeColor: function(colorFrom, colorTo) {
		for(var i = 0; i < this.originalColors.length; i++) {
			if(this.originalColors[i] == colorFrom)
				this.currentColors[i] = colorTo;
		}
	},
...
/**
 * @class fabric.Image.filters.SetColor
 * @memberOf fabric.Image.filters
 */
fabric.Image.filters.SetColor = fabric.util.createClass(fabric.Object, /** @scope fabric.Image.filters.SetColor.prototype */ {
...
fabric.Image.filters.SetColor.fromObject = function() {	
	return new fabric.Image.filters.SetColor();
};

/**
 * @class fabric.Image.filters.ColorChange
 * @memberOf fabric.Image.filters
 */
fabric.Image.filters.ColorChange = fabric.util.createClass(fabric.Object, /** @scope fabric.Image.filters.ColorChange.prototype */ {
  ...
  initialize: function(options) {
	  this.set('colorTo', options.colorTo);
	  this.set('colorFrom', options.colorFrom);
	  this.set('origImageData', options.origImageData);
  },
  ...
  applyTo: function(canvasEl) {
    ...
    var context = canvasEl.getContext('2d'),
        imageData = context.getImageData(0, 0, canvasEl.width, canvasEl.height),            
        r1, g1, b1, r2, g2, b2,
		colorFrom = this.get('colorFrom'),
		colorTo = this.get('colorTo'),
		origImageData = this.get('origImageData');		

	r1 = hextorgb(colorFrom)[0];
	g1 = hextorgb(colorFrom)[1];
	b1 = hextorgb(colorFrom)[2];

	r2 = hextorgb(colorTo)[0];
	g2 = hextorgb(colorTo)[1];
	b2 = hextorgb(colorTo)[2];

    for (var i = 0, len = imageData.data.length; i < len; i += 4) {
		if(origImageData.data[i] == r1 && origImageData.data[i+1] == g1 && origImageData.data[i+2] == b1) {
			imageData.data[i] = r2;
...
fabric.Image.filters.ColorChange.fromObject = function() {	
	return new fabric.Image.filters.ColorChange();
};
...
(function(global) {
   ...
   * @extends fabric.Object
   */
  fabric.Alias = fabric.util.createClass(fabric.Object, /** @scope fabric.Alias.prototype */ {
    ...
    /**
     * Creates `stateProperties` list on an instance, and adds `fabric.Alias -specific ones to it
     ...
     */
    toString: function() {
      return '#<fabric.Alias (' + this.complexity() +
    ...
    _render: function(context) {
		   ...
		   this.strokeWidth = this.fontSize / 30;		   
		   if(this.isBorder) context.strokeStyle = "#" + this.borderColor;
           context.font = this.fontSize + 'px ' + '"' + this.fontFamily + '"';		  
           this.transform(context);		
		   this.width = context.measureText(this.text).width + this.letterSpacing * (this.text.length - 1);
           this.height = this.fontSize;
		   var x = -this.width / 2;
		   var y = this.height * 5 / 12;
		   for(var i=0; i<this.text.length; i++) {			 
		     context.fillText(this.text[i], x, y);
		     if(this.isBorder) context.strokeText(this.text[i], x, y);
			 x += context.measureText(this.text[i]).width + this.letterSpacing;
		   }
    ...
    setColor: function(value) {
      this.set('fill', value);
	  this.fillColor = value.substring(1, value.length);
      return this;
    },

	/**
     * Sets "border" of an instance
     * @method setBorder
     * @param {String} value
     * @return {fabric.Alias} thisArg
     * @chainable
     */
    setBorder: function(value, b) {
	  this.isBorder = b;
	  if(b) {
		this.borderColor = value.substring(1, value.length);	  
		this.setCoords();
	  }  else {
  ...
  /**
   * Returns fabric.Alias instance from an object representation
   * @static
   * @method fromObject
   * @param {Object} object to create an instance from
   * @return {fabric.Alias} an instance
   */
  fabric.Alias.fromObject = function(object) {
    return new fabric.Alias(object.text, clone(object));
  };

  /**
   * Returns fabric.Alias instance from an SVG element (<b>not yet implemented</b>)
   * @static
   * @method fabric.Alias.fromElement
   * @return {fabric.Alias} an instance
   */
  fabric.Alias.fromElement = function(text, options, callback) {
    if(callback) {
		callback(new fabric.Alias(text, options));
	}
  };
   ...
   * @return {fabric.Text} an instance
   */
  fabric.Text.fromElement = function(text, options, callback) {
    if(callback) {
		callback(new fabric.Text(text, options));
	}
  };

  fabric.CircleText = fabric.util.createClass(fabric.Object, /** @scope fabric.CircleText.prototype */ {
    ...
    _render: function(context) {
	  ...
	  var numRadsPerLetter = (this.radius + (this.letterSpacing * this.radius / 20)) / (this.text.length);
	  var startRotation = -(this.radius + (this.letterSpacing * this.radius / 20)) / 2;
	  var radius = this.text.length * 10;	
	  
	  this.width = (radius + this.fontSize) * 2;
      this.height = (radius + this.fontSize) * 2;

	  context.save();
	  context.rotate(startRotation + numRadsPerLetter / 3);

      for(var i=0;i<this.text.length;i++){
        context.save();
        context.rotate(i*numRadsPerLetter);

        context.fillText(this.text[i],0,-radius);
	    if(this.isBorder) context.strokeText(this.text[i],0,-radius);
        context.restore();
	  }
     ...
     * @method setColor
     * @param {String} value
     * @return {fabric.CircleText} thisArg
     * @chainable
     */
    setColor: function(value) {
      this.set('fill', value);
   ...
   * @method fromObject
   * @param {Object} object to create an instance from
   * @return {fabric.CircleText} an instance
   */
  ...
  /**
   * Returns fabric.CircleText instance from an SVG element (<b>not yet implemented</b>)
   * @static
   * @method fabric.CircleText.fromElement
   * @return {fabric.CircleText} an instance
   */
  fabric.CircleText.fromElement = function(element) {
    // TODO (kangax): implement this
  };

  fabric.VerticalText = fabric.util.createClass(fabric.Object, /** @scope fabric.VerticalText.prototype */ {
     ...
     * @param {String} text
     * @param {Object} [options]
     * @return {fabric.VerticalText} thisArg
     */
    initialize: function(text, options) {
      this._initStateProperties();
      this.text = text;
    ...
    /**
     * Creates `stateProperties` list on an instance, and adds `fabric.VerticalText -specific ones to it
     * (such as "fontFamily", "fontWeight", etc.)
     * @private
     * @method _initStateProperties
     */
    ...
    _render: function(context) {
	  ...
	  this.width = context.measureText(this.text[0]).width * 2;
      this.height = (this.fontSize + this.letterSpacing) * this.text.length - this.letterSpacing + this.fontSize / 2;  

	  var x=0, y=-this.height / 2 - this.letterSpacing;
   
      for(var i=0;i<this.text.length;i++){
		context.save();
		x = -context.measureText(this.text[i]).width / 2;
  ...
  /**
   * Returns fabric.VerticalText instance from an object representation
   * @static
   * @method fromObject
   * @param {Object} object to create an instance from
   * @return {fabric.VerticalText} an instance
   */
  fabric.VerticalText.fromObject = function(object) {
    return new fabric.Text(object.text, clone(object));
  };

  /**
   * Returns fabric.VerticalText instance from an SVG element (<b>not yet implemented</b>)
   * @static
   * @method fabric.VerticalText.fromElement
   * @return {fabric.VerticalText} an instance
   */
  fabric.VerticalText.fromElement = function(element) {
    // TODO (kangax): implement this
  };
